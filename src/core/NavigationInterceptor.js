...
class NavigationInterceptor {
  ...
  wrapHistoryMethods() {
        ...
        setTimeout(() => {
          self.handleNavigation(
            methodName === 'pushState'
              ? NavigationEventType.PUSH_STATE
              : NavigationEventType.REPLACE_STATE,
            url
          );
        }, 0);
    ...
    this.historyRewrapInterval = setInterval(() => {
      ...
      if (
        !currentPush.includes('handleNavigation') &&
        !currentPush.includes('__navigationInterceptor')
      ) {
        this.log('pushState was overridden, re-wrapping...');
        history.pushState = wrapMethod('pushState', history.pushState.bind(history));
      }
      if (
        !currentReplace.includes('handleNavigation') &&
        !currentReplace.includes('__navigationInterceptor')
      ) {
