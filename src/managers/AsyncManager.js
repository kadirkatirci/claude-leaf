...
class AsyncManager {
    ...
    async waitForElement(selector, timeout = 10000, root = document.body) {
        ...
        return new Promise((resolve, reject) => {
            ...
            }, timeout, `Wait for element: ${selector}`);
    ...
    async withTimeout(asyncFn, timeout = 30000, description = 'Async operation') {
        ...
        return Promise.race([
            ...
            new Promise((_, reject) => {
                ...
                }, timeout, `Timeout for: ${description}`);
            })
        ]);
    }
    ...
    async retry(asyncFn, maxRetries = 3, initialDelay = 1000, description = 'Async operation') {
        ...
        throw new Error(`Failed after ${maxRetries} retries: ${description}. Last error: ${lastError?.message}`);
    }
        ...
        return (...args) => {
            ...
            if (timeSinceLastExec >= delay) {
                ...
                timeoutId = this.setTimeout(() => {
                    ...
                    fn.apply(this, args);
                }, remainingDelay, `Throttle for function`);
            }
        };
    }
